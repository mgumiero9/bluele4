...
static void wakeup_writer(struct mgmt *mgmt)
{
	...
	printf("Mgmt queue not empty\n");

	if (mgmt->writer_active)
	{
	    printf("Mgmt queue writer active error\n");
	    return;
	}
...
struct mgmt *mgmt_new(int fd)
{
    ...
    printf("Starting replying queue\n");
    ...
    printf("Starting notifying queue\n");
    ...
    printf("Starting io set read queue\n");
    ...
    printf("Mgmt write active disabled\n");
	mgmt->writer_active = false;
				...
				void *user_data, mgmt_destroy_func_t destroy)
{
    ...
    printf("Request Pushing\n");

	if (!queue_push_tail(mgmt->request_queue, request)) {
	    printf("Erro Queue Mgmt Request\n");
		free(request->buf);
		free(request);
		return 0;
	}
	printf("Request Pushed\n");
